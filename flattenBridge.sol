...
// File contracts/BridgePixel.sol
...
contract BridgePixel is Ownable, ReentrancyGuard {
    ...
    address public oppositePixelTokenAddress;
    iBridgePixelToken public oppositepixelBridgeToken;

    address public oppositePixelBridgeContract;

    bytes32 public attestationTypeEVM;
    bytes32 public oppositeSourseID; // name of opposite chain upper case in bytes32
    bytes32 public currentSourseID;
    uint16 public oppositeRequiredConfirmation;
    uint64 public currentLowestUsedTimestamp; //18446744073709552000

    // mapping(bytes32 => bool) public openWithdrawRequest;
    // mapping(bytes32 => bool) public closeWithdrawRequest;
    ...
    constructor(string memory _name) Ownable(msg.sender) {
        // if(block.chainid == 14){
        //     name = "Pixel bridge on Flare";
        //     thisChainID = 14;
        //     oppositeChainID = 19;
        // } else if (block.chainid == 19){
        //     name = "Pixel bridge on Songbird";
        //     thisChainID = 19;
        //     oppositeChainID = 14;
        // } else {
        //     revert();
    ...
    address public oppositeBridge;

    uint256 public fee; // fee of 10000
    uint256 constant denom = 10000;

    

    uint256 public totalBalanceNative;
    // uint256 public totalBalancePixel;
    // mapping(address => uint256) public totalBalanceERC20;
    ...
    function SetOppostePixelBridgeToken(address _oppositePixelBridgeToken) external onlyOwner(){
        oppositePixelTokenAddress = _oppositePixelBridgeToken;
        oppositepixelBridgeToken = iBridgePixelToken(_oppositePixelBridgeToken);
    }
    ...
    function SetCurrentSourceId(bytes32 _sourceId) external onlyOwner(){
        // FLR
    ...
    function SetOppositeBridgeContractAddress(address _oppositeBridge) external onlyOwner(){
        oppositeBridge = _oppositeBridge;
    }
    ...
    function requestWithdraw(bool isNative, bytes32 _txHash, uint256 _blockNumber, uint256 _timestamp, uint256 _valueTotal, uint256 _valueToWithdraw) external payable nonReentrant {
        ...
        // require(openWithdrawRequest[keccak256(abi.encode(newResp))] == false, "request is already open");
        // openWithdrawRequest[keccak256(abi.encode(newResp))] = true;

        // Attestation hash
        // keccak256(abi.encode(response));
        // ere the response is a struct of type Response. Hashes of the confirmed attestation are used to construct the Merkle tree and the Merkle root is used for voting in the attestation round.
